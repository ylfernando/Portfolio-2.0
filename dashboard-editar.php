...
<body>
    ...
    <section class="section">
        ...
        <div class="add-projeto-area">
            <h1 class="h1-projeto">Editar projeto</h1>
            <div class="formulario-area">
                <form style="display: flex; flex-direction:column;" class="form" enctype="multipart/form-data" method="POST">
                <?php
                    if(isset($_POST['empresa']) && isset($_POST['url'])) {
                        $empresa = filter_input(INPUT_POST, 'empresa', FILTER_SANITIZE_SPECIAL_CHARS);
                        $url = filter_input(INPUT_POST, 'url', FILTER_VALIDATE_URL);
                        if(isset($_FILES['logo']) && !empty($_FILES['logo'])) {
                            $logo = $_FILES['logo'];
                            $p -> addImagemProjeto($_GET['id'], $logo);
                        } else {
                            $logo = [];
                        }
                        $p -> editarProjeto($empresa, $url, $_GET['id'], $logo);


                    }
                    if(isset($_GET['id']) && !empty($_GET['id'])) {
                        $projeto = $p -> getProjeto($_GET['id']);
                    }
                ?>
                    <label class="label">
                    ...
                    <label class="label">
                        <h2 class="h2-form">Logo da empresa do cliente:</h2>
                        <input type="file" class="input" name="logo[]"/>
                    </label> <br> <br>
                    <input class="btn-add" type="submit" value="Editar projeto">
                </form>
            </div>
        </div>
        </div>
